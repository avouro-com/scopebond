...
| `gateway` | The policy-enforcement proxy (HTTP + MCP ingress): prevents, countersigns, logs, kill switch. `npx scopebond-gateway`. | **early** (enforce/monitor/approval via verify; Ed25519 `scopebond:receipt`s; kill switch; in-memory store; tested) |
